...
IMAGE_PATH = '/root/data/' + dataset_name + '/train_images/'
MASK_PATH = '/root/data/' + dataset_name + '/train_annot/'
IMAGE_PATH_test = '/root/data/' + dataset_name + '/test_images/'
MASK_PATH_test = '/root/data/' + dataset_name + '/test_annot/'
...
all_ids = np.sort(df['id'].values)


val_ratio = 0.2
val_step = int(1 / val_ratio) 
X_val = all_ids[::val_step]
XX_train = np.array([x for x in all_ids if x not in X_val]) 

labeled_ratio = 0.2
labeled_step = int(1 / labeled_ratio) 

XX_train = np.sort(XX_train) 

X_train = XX_train[::labeled_step]
X_untrain = np.array([x for x in XX_train if x not in X_train])
...
unlabeled_loader = DataLoader(unlabeled_train_set, batch_size=batch_size, num_workers = 0, pin_memory = True, shuffle=True)
val_loader = DataLoader(val_set, batch_size=batch_size, num_workers = 0, pin_memory = True, shuffle=True)  
