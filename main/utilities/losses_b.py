...
class DiceLoss(nn.Module):
    ...
    def forward(self, y_pr, y_gt):
        return 1 - f_score(y_pr, y_gt, beta=1., eps=self.eps, threshold=None, activation='sigmoid') #self.activation)
...
loss_calc = BCEDiceLoss()


def super_loss(logit_S1, logit_S2, labels_S1, labels_S2):
    loss1 = loss_calc(logit_S1, labels_S1)
    loss2 = loss_calc(logit_S2, labels_S2)

    return loss1 + loss2


def super_loss_1(logit_S1, labels_S1):
    loss1 = loss_calc(logit_S1, labels_S1)
...
def kl_loss(pr, gt):
    ...
    input_log = torch.log(pr)
    target_log = gt
    kl_div = F.kl_div(input_log, target_log, reduction='mean')
    return kl_div
